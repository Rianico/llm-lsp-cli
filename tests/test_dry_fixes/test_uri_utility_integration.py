"""Tests for uri_to_relative_path integration (Issue 2b)."""
...
class TestUriUtilityIntegration:
    ...
    def test_diagnostic_cache_uses_uri_utility(self, tmp_path: Path) -> None:
        """DiagnosticCache._uri_to_relative_path should delegate to utils.uri."""
        cache = DiagnosticCache(tmp_path)
        test_uri = (tmp_path / "test.py").as_uri()

        # Mock at the location where it's imported (in cache module)
        with patch("llm_lsp_cli.lsp.cache.uri_to_relative_path") as mock_util:
            mock_util.return_value = "test.py"

            cache._uri_to_relative_path(test_uri)

            mock_util.assert_called_once()

    def test_lsp_client_uses_uri_utility(self, tmp_path: Path) -> None:
        """LSPClient._uri_to_relative_path should delegate to utils.uri."""
        ...
        with patch("llm_lsp_cli.utils.uri.uri_to_relative_path") as mock_util:
            mock_util.return_value = "test.py"

            client._uri_to_relative_path(test_uri)
    ...
    def test_both_consumers_produce_same_result(self, tmp_path: Path) -> None:
        ...
        cache_result = cache._uri_to_relative_path(test_uri)
        client_result = client._uri_to_relative_path(test_uri)
