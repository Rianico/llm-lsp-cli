...
class TestConfigManagerFacadeEdgeCases:
    ...
    def test_config_manager_ensure_project_dir_idempotent(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, reset_xdg_paths: None
    ) -> None:
        """ensure_project_dir() is idempotent."""
        # Arrange
        ...
        # Act: Call twice
        dir1 = ConfigManager.ensure_project_dir(str(workspace), base_dir=runtime_dir)
        dir2 = ConfigManager.ensure_project_dir(str(workspace), base_dir=runtime_dir)

        # Assert
        assert dir1 == dir2
        assert dir1.exists()

    def test_config_manager_get_runtime_base_dir_strips_subdir(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, reset_xdg_paths: None
    ) -> None:
        """get_runtime_base_dir() strips the llm-lsp-cli suffix."""
        # Arrange
        runtime_dir = tmp_path / "runtime"
        ...
        # Act
        base_dir = ConfigManager.get_runtime_base_dir()

        # Assert
        assert str(base_dir) == str(runtime_dir)
        assert "llm-lsp-cli" not in str(base_dir)
