...
The diagnostic cache implements mtime-based invalidation for LSP diagnostic responses (ADR-0008) with split channels for document and workspace diagnostics (ADR-0029).
...
class FileState:
    ...
    last_result_id: str | None = None       # Server's diagnostic version
    is_open: bool = False                   # Whether didOpen was sent
    document_diagnostics: list[dict]        # From publishDiagnostics + textDocument/diagnostic
    workspace_diagnostics: list[dict]       # From workspace/diagnostic via $/progress
    has_workspace_diagnostics: bool = False # True if workspace diags explicitly written
...
|--------|---------|---------|
| `is_stale(uri, incoming_mtime) -> bool` | Both | Returns `incoming_mtime > stored_mtime` |
| `on_did_open(uri, mtime)` | Both | Sets `is_open=True`, increments version |
| `update_document_diagnostics(uri, diagnostics, result_id)` | Document | Stores after publishDiagnostics or textDocument/diagnostic |
| `update_workspace_diagnostics(uri, diagnostics)` | Workspace | Stores after workspace/diagnostic via $/progress |
| `get_document_diagnostics(uri) -> list[dict]` | Document | Returns cached document diagnostics |
| `get_workspace_diagnostics_for_uri(uri) -> list[dict]` | Workspace | Returns cached workspace diagnostics for a file |
| `get_all_workspace_diagnostics() -> list[WorkspaceDiagnosticItem]` | Workspace | Returns all files with has_workspace_diagnostics=True |
| `get_diagnostics(uri) -> list[dict]` | Document | Backward-compatible alias for get_document_diagnostics |

### Cache Invariants

1. **mtime is ground truth** - Staleness determined solely by mtime comparison (applies to both channels)
...
5. **Channels never cross-write** - document_diagnostics written only by document-level sources; workspace_diagnostics written only by workspace-level sources (ADR-0029)
6. **has_workspace_diagnostics flag** - Distinguishes "never received" from "explicitly empty"; get_all_workspace_diagnostics skips files without this flag (ADR-0029)
