...
class LanguageServerConfig(BaseModel):
    ...
    initialize_params_file: str | None = Field(
        default=None,
        description="Path to initialize params JSON file"
    )
...
class ClientConfig(BaseModel):
    ...
    pid_file: str = Field(
        default="$XDG_RUNTIME_DIR/llm-lsp-cli/daemon.pid",
        description="PID file location"
    )
    log_file: str = Field(
        default="$XDG_STATE_HOME/llm-lsp-cli/daemon.log",
        description="Daemon log file"
    )

    # Language servers
    languages: dict[str, LanguageServerConfig] = Field(
        default_factory=dict,
        description="Language-specific server configurations"
    )

    # Global settings
    trace_lsp: bool = Field(
        default=False,
        description="Enable LSP communication tracing"
    )
    timeout_seconds: int = Field(
        default=30,
        description="Default timeout for LSP requests"
    )
